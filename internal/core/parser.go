...
type ParsedNote struct {
	...
	// Indicate if the note has been generated by a processor (ex: a generator)
	Generated bool
...
func (p *ParsedFile) extractNotes() ([]*ParsedNote, error) {
	var notes []*ParsedNote              // All notes collected until now
	var noteSections []*markdown.Section // Sections matching these notes
	// NB: Both slices must be in the same order with the same length to be able to determine parents
	...
	for _, section := range sections {
		...
		// Append the note (or several when using generators)
		notes = append(notes, parsedNotes...)
		// Duplicate the section for each generated note to be able to determine parents for all of them
		for range parsedNotes {
