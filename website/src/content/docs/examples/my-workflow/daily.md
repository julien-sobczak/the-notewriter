...
I use notes to limit interruptions. When I think about something I need to do, read, check, buy or when I have a new idea about a project, I write a short Google Keep note (usually a few words or just a link). The motivation is to get everything out of my mind as soon as possible.
...
1. **Is it relevant?** Several notes are just deleted without any action. What seems interesting is no longer valuable just a few days later.
2. **Is it actionable?** When?
  ...
  * Later? If the note is useful for a project (in-progress, planned, or future), I put the note inside the project file (I keep a directory per project with an `index.md` inside). If the note is an inspiring quote, I move it to my commonplace book (in the directory `thoughts/on-<topic>` in the file best matching the topic).

Example of common daily notes:

* A quote ▶ move to `references/persons/<name>.md` + add tags, attributes like `@source`, etc.
* An idea ▶ move to `projects/<project>/index.md` or create a new directory into `projects/`. It can be the idea for a new blog post or just a seed to reread during a future blog post.
* A book ▶ add to `todo/maybe/read.md` + add a rating to evaluate my interest for it. Same for a video (`todo/maybe/watch.md`).
