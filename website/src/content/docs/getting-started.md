...
Things becomes more interesting. The command `add` parses all Markdown files to extract objects (like notes and flashcards) and add them to the index. The objects are also saved in a SQLite database for performance reasons while supporting full-text searches.
