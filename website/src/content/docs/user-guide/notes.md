...
## Quote: Me on Nothing Interesting
...
These optional comments are useful to explain why a note like a quote resonates, or to summarize the key idea in your own words. These comments are highlighted differently (and can be omitted) when rendered in _The NoteWriter Desktop_.
...
Notes are the building block of _The NoteWriter_. When adding a file using `nt add`, the application will parsed the Markdown to search for headings matching one of the note type to extract a new object for every note. All objects present in a file are stored in the same pack file under `.nt/objects`.
