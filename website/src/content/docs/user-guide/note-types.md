...
When writing a new book review, the attribute `draft` can be omitted. The value `true` applies automatically. To publish the review (for example using a [hook](./hooks)), the attribute `draft` must be explicitely defined to `false`. No draft is published before being ready.
...
When using the type `List`, the note will be displayed differently in the desktop application:

TODO screenshot

The application also supports filtering on attributes (and tags) present on list items. These attributes are called "inline" (vs "block" when they are declared in Markdown headings or in a specific line).
