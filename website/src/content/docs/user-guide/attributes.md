...
Custom Attributes, in addition to [custom note types](./note-types) and the [linter](./linter) are one the key features to manage a large collection of notes over time.
...
Only properties `name` and `type` are required (see the above [list of types](#attribute-types)).
...
* `inherit`: `false` to prevent an attribute to be inherited by sub-notes. Attributes are inherited by default.
...
Using shorthands, the attribute `@rating` can be declared using Unicode characters:
...
Shorthands are particularly useful during [journaling](../use-cases/journaling.md), like when using the Bullet Journal Method.
