...
* `easeFactor` (type `number`): The default ease factor for new cards. Used to calculate intervals for reviewing. (Ex: `2.5`)
* `steps` (type `string[]`) An array of interval steps for learning (ex: `['1m', '10m', '1d']`). These define the progression for cards in the learning queue.
...
These two attributes are useful to boost the scheduling interval between reviews depending your interest on the topic. It's usually easier to remember on topics where are naturally more enthusiastic about it. Thes attributes will often be used in the Front Matter:
...
:::info[Study on the go]

[The support for remotes](../user-guide/remotes.md) is minimal. The long-term goals is to have various remotes to carry your notes with you on the go, with the possibility to study the flashcards on your phone.
